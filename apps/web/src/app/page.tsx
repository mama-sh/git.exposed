...
export default function Home() {
  ...
  useEffect(() => {
    if (animPhase < 5) { setDisplayScore(100); return; }
    let val = 100;
    const id = setInterval(() => {
      val -= 4;
      if (val <= 12) { val = 12; clearInterval(id); }
      setDisplayScore(val);
    }, 18);
        ...
        <div className="max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-center py-20">
          ...
          <div className="hidden md:flex items-center justify-center">
            <div className="relative">
              ...
              <div className="bg-ds-muted/80 border border-ds-border/40 rounded-2xl p-6 w-[340px] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.4)] relative overflow-hidden">
                ...
                {animPhase === 1 && (
                  <div
                    ...
                    style={{
                      background: 'linear-gradient(90deg, transparent, #ef4444 25%, #f97316 50%, #ef4444 75%, transparent)',
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-xs ml-2 font-mono">
                    {animPhase === 1 && <span className="text-red-400">scanning<span className="animate-pulse">▌</span></span>}
                ...
                <div className="space-y-3">
                  <div className={cn('flex items-center gap-2 transition-all duration-300 ease-out', animPhase >= 2 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1.5')}>
                    <span className={cn('text-[0.6rem] font-bold uppercase px-1.5 py-0.5 rounded bg-red-700 text-white transition-transform duration-200', animPhase >= 2 ? 'scale-100' : 'scale-75')}>critical</span>
                    <span className="text-sm text-slate-300">Hardcoded API key</span>
                  </div>
                  <div className={cn('flex items-center gap-2 transition-all duration-300 ease-out', animPhase >= 3 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1.5')}>
                    <span className={cn('text-[0.6rem] font-bold uppercase px-1.5 py-0.5 rounded bg-orange-700 text-white transition-transform duration-200', animPhase >= 3 ? 'scale-100' : 'scale-75')}>high</span>
                    <span className="text-sm text-slate-300">SQL injection risk</span>
                  </div>
                  <div className={cn('flex items-center gap-2 transition-all duration-300 ease-out', animPhase >= 4 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1.5')}>
                    <span className={cn('text-[0.6rem] font-bold uppercase px-1.5 py-0.5 rounded bg-yellow-700 text-white transition-transform duration-200', animPhase >= 4 ? 'scale-100' : 'scale-75')}>medium</span>
                    <span className="text-sm text-slate-300">Outdated dependency</span>
                  </div>
                </div>

                {/* Score — fades in and counts down */}
                <div className={cn('mt-4 pt-3 border-t border-ds-border/30 transition-all duration-400 ease-out', animPhase >= 5 ? 'opacity-100' : 'opacity-0')}>
              ...
              <div className={cn('absolute -inset-8 blur-3xl rounded-full -z-10 transition-all duration-700', animPhase >= 2 ? 'bg-red-500/8' : 'bg-red-500/4')} />
            </div>
          </div>
        </div>
      </div>
